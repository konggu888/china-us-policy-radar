...
def call(prompt,max_tokens=3000):
    ...
    body={'model':MODEL,'messages':[{'role':'system','content':'你是全球战略政策情报分析员。只根据提供的公开情报做归纳，不把推测写成事实；明确区分官方立场、媒体报道和分析判断。输出必须是合法JSON。'}, {'role':'user','content':prompt}], 'stream':False,'max_tokens':max_tokens,'response_format':{'type':'json_object'}}
    req=urllib.request.Request('https://api.deepseek.com/chat/completions',data=json.dumps(body,ensure_ascii=False).encode(),headers={'Authorization':'Bearer '+KEY,'Content-Type':'application/json','User-Agent':'China-US-Global-Intelligence-Radar/DeepSeek-1.0'})
    try:
        raw=urllib.request.urlopen(req,timeout=120).read().decode('utf-8'); obj=json.loads(raw); text=obj['choices'][0]['message']['content']; return json.loads(text)
    except Exception as e: print('DeepSeek error:',type(e).__name__); return None

def classify(rows):
    todo=[n for n in rows if not n.get('ai_category')][:60]
    if not todo:return 0
    evidence=[]
    for i,n in enumerate(todo):evidence.append({'id':i,'title':n.get('titleZh') or n.get('title',''),'original':n.get('title',''),'region':n.get('region'),'category':n.get('cat'),'source':n.get('sourceOrg') or n.get('source'),'url':n.get('url')})
    prompt='请重新判断以下新闻的标准分类。每条必须从13类中选1类，并判断region为china/us/global，importance为1-100，是否为明确政策行动action，以及给出不超过25字中文reason；英文标题同时给出titleZh。不要改变URL。\n13类：'+','.join(CATS)+'\n数据：'+json.dumps(evidence,ensure_ascii=False)
    ans=call(prompt,5000)
    ...
    for x in ans['items']:
        ...
        n['ai_action']=bool(x.get('action',False)); n['ai_reason']=str(x.get('reason',''))[:100]
...
def digest(rows,days):
    ...
    top=sorted(rs,key=lambda n:(n.get('ai_importance',0),n.get('importance_score',0)),reverse=True)[:120]
    return {'count':len(rs),'categories':dict(cats),'regions':dict(regs),'importance_levels':dict(levels),'top':[{'title':n.get('titleZh') or n.get('title'),'region':n.get('region'),'category':n.get('cat'),'importance':n.get('ai_importance',n.get('importance_score',0)),'source':n.get('sourceOrg') or n.get('source'),'url':n.get('url')} for n in top]}

def make_summary(rows,days,label,market):
    d=digest(rows,days)
    prompt=f'''请根据下面{label}公开情报做一份中文战略情报摘要。必须输出JSON，字段：headline、executive_summary、china（正在做什么/对外政策/对内政策/重点板块）、us（正在做什么/对外政策/对内政策/重点板块）、global（全球正在发生什么/区域热点/主要政策方向）、us_market（上涨板块/下跌板块/观察依据）、key_risks、key_opportunities、signals_to_watch、confidence、evidence_count。不要预测具体事件概率，不要提供个股买卖建议；“上涨/下跌板块”只能根据给出的市场数据。对新闻事实与分析判断分开。\n情报统计：{json.dumps(d,ensure_ascii=False)}\n美国板块市场数据：{json.dumps(market,ensure_ascii=False)}'''
    return call(prompt,7000)
...
def main():
    ...
    if not KEY:
        print('DEEPSEEK_API_KEY not configured; archive/classification pipeline remains active'); return 0
    ...
    print('DeepSeek',period,'summary generated; classified',classified,'items')
    return 0
