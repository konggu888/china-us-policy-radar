...
UA='China-US-Global-Intelligence-Radar/BroadCollector-3.0'
...
# IMPORTANT: do not use a tiny regional/category quota. Keep a large rolling working set; archive_news.py persists monthly history.
# The working set is capped only as a safety valve, never as an analytical sampling rule.
...
health.update({'updated':stamp,'method':'Google News public RSS discovery + existing official sources; no analytical quota','total_collected':len(selected),'new_discovery_count':total,'category_counts':bycat,'region_counts':byregion,'sources':health})
OUT.write_text(json.dumps(selected,ensure_ascii=False,indent=2),encoding='utf-8'); HEALTH.write_text(json.dumps(health,ensure_ascii=False,indent=2),encoding='utf-8'); print('broad discovery:',total,'items; working set:',len(selected),'categories:',bycat,'regions:',byregion)