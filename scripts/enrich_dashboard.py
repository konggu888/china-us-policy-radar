...
ROOT=Path(__file__).resolve().parents[1]; DATA=ROOT/'data'; OUT=DATA/'dashboard.json'; UA='China-US-Global-Intelligence-Radar/5.2'
def fetch_json(url):
    try:
...
try: market_data=json.loads((DATA/'market_sectors.json').read_text(encoding='utf-8'))
except Exception: market_data={}
...
def enrich(n):
    ...
    n['impact_industry']=list(dict.fromkeys(inds))[:4]; n['short_term']='关注市场预期、政策执行与相关行业价格/订单变化' if high else '主要观察政策落地和市场情绪变化'; n['long_term']='观察产业链、资本流向、贸易结构与政策持续性' if (high or comp) else '观察后续政策是否形成连续措施'; n['watch_data']=['官方后续文件','行业数据','价格/订单','企业公告']; n['policy_level']='高优先级' if n.get('sourceType')=='official' and high else '常规跟踪'; return n
news=[enrich(n) for n in news]; (DATA/'news.json').write_text(json.dumps(news,ensure_ascii=False,indent=2),encoding='utf-8')
markets=[('USD/CNY','CNY=X'),('上证指数','000001.SS'),('深证成指','399001.SZ'),('沪深300','000300.SS'),('中证500','000905.SS'),('创业板指','399006.SZ'),('科创50','000688.SS'),('标普500','^GSPC'),('美国10年期收益率','^TNX'),('布伦特原油','BZ=F'),('黄金','GC=F'),('VIX','^VIX')]
market_rows=[]
...
# Detailed market layer: exact recent returns from the market collector, not AI-generated.
us_sectors=market_data.get('sectors',{}); a_share=market_data.get('a_share',{})
us_rank=sorted([{'name':n,**v} for n,v in us_sectors.items()],key=lambda x:x.get('weekly_pct') if x.get('weekly_pct') is not None else -999,reverse=True)
a_rank=sorted([{'name':n,**v} for n,v in a_share.items()],key=lambda x:x.get('weekly_pct') if x.get('weekly_pct') is not None else -999,reverse=True)
market_highlights={'us_sector_risers':us_rank[:5],'us_sector_fallers':list(reversed(us_rank[-5:])) if us_rank else [],'a_share_risers':a_rank[:5],'a_share_fallers':list(reversed(a_rank[-5:])) if a_rank else [],'asof':market_data.get('updated','')}
obj={'updated':now.strftime('%Y-%m-%d %H:%M UTC'),'market':market_rows,'us_sector_market':us_sectors,'a_share_market':a_share,'market_highlights':market_highlights,'globalization_temperature':{'score':global_temp,'label':'开放信号偏强' if global_temp>=60 else '限制信号偏强' if global_temp<=40 else '冷热交织','method':'公开政策/贸易/投资/制裁/出口管制信号的相对温度；不是官方指标。'},'policy_direction':{'china':direction('china'),'us':direction('us'),'global':direction('global')},'policy_language':policy_language,'policy_source_matrix':source_matrix,'policy_chain':policy_chain,'policy_comparison':comparison,'today_focus':focus}
OUT.write_text(json.dumps(obj,ensure_ascii=False,indent=2),encoding='utf-8'); print('wrote',OUT,'US sectors',len(us_sectors),'A-share indices',len(a_share))
