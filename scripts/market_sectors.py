...
UA='Mozilla/5.0 (compatible; China-US-Global-Intelligence-Radar/Market-Collector-3.0)'
...
def eastmoney_boards():
    ...
    try:
        req=urllib.request.Request(EM+'?'+urllib.parse.urlencode(params),headers={'User-Agent':'Mozilla/5.0','Referer':'https://data.eastmoney.com/'})
        ...
        for x in rows:
            ...
            out.append({'code':x.get('f12'),'name':name,'change_pct':round(p,2),'price':x.get('f2'),'change':x.get('f4'),'up_count':x.get('f104'),'down_count':x.get('f105'),'leader':x.get('f128') or ''})
...
payload={'updated':datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC'),'method':'US sector ETF proxy + Eastmoney A-share industry board ranking + A-share major index observations','sectors':us,'a_share':a_share,'a_share_industry_boards':boards,'a_share_board_status':status,'weekly_risers':[n for n,v in rank if (v.get('weekly_pct') or 0)>0],'weekly_fallers':[n for n,v in reversed(rank) if (v.get('weekly_pct') or 0)<0],'a_share_weekly_risers':[n for n,v in arank if (v.get('weekly_pct') or 0)>0],'a_share_weekly_fallers':[n for n,v in reversed(arank) if (v.get('weekly_pct') or 0)<0],'a_share_board_risers':[x['name'] for x in boards[:5]],'a_share_board_fallers':[x['name'] for x in boards[-5:][::-1]]}
OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding='utf-8'); print('market sectors:',len(us),'A-share indices:',len(a_share),'A-share industry boards:',len(boards),'status:',status)
