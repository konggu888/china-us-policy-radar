...
def get(ticker):
    ...
    try:
        req=urllib.request.Request(url,headers={'User-Agent':UA}); obj=json.loads(urllib.request.urlopen(req,timeout=15).read().decode()); r=obj['chart']['result'][0]; q=r['indicators']['quote'][0]; closes=[x for x in q['close'] if x is not None]; ts=[x for x,c in zip(r['timestamp'],q['close']) if c is not None]
...
def eastmoney_boards():
    ...
    for host in EM_HOSTS:
        try:
            req=urllib.request.Request(host+'?'+urllib.parse.urlencode(params),headers={'User-Agent':'Mozilla/5.0','Referer':'https://data.eastmoney.com/'}); obj=json.loads(urllib.request.urlopen(req,timeout=20).read().decode('utf-8','ignore')); rows=obj.get('data',{}).get('diff',[]) or []; out=[]
            for x in rows:
