...
UA='China-US-Global-Intelligence-Radar/3.0'
...
for name,pat in COUNTRY_NAMES.items():
    ...
    prev=oldmap.get(CODE_MAP[name],{}).get('score',0)
...
# Timeline: highest-risk evidence items.
...
# Alerts: anomaly signals only, not predictions.
alerts=[]
for c in country_rows:
    if c['delta']>=15:
        alerts.append({'severity':'高','type':'国家风险跃升','title':f"{c['name']} 风险指数单日上升 {c['delta']} 点",'reason':f"当前 {c['score']}/100；主要驱动：{c['driver']}；相关报道 {c['articles']} 条。",'country':c['name'],'time':stamp,'evidence':c['latest']})
...
if prev_total>=10 and len(uniq)>=prev_total*1.8:
    alerts.append({'severity':'中','type':'情报量异常','title':'今日公开情报量较前一日明显放大','reason':f'今日 {len(uniq)} 条，前一日 {prev_total} 条，超过 1.8 倍阈值。','time':stamp,'evidence':'news.json'})
if prev_high>=5 and cur_high>=prev_high*2:
    alerts.append({'severity':'高','type':'高风险信号放大','title':'高风险情报量较前一日翻倍','reason':f'今日高/极高 {cur_high} 条，前一日 {prev_high} 条。','time':stamp,'evidence':'news.json'})
cat_counts=Counter(x['cat'] for x in uniq)
if cat_counts['中美竞争']>=5 and cat_counts['军事']>=5 and cat_counts['能源安全']>=5:
    alerts.append({'severity':'高','type':'多领域联动','title':'中美竞争、军事与能源安全信号同时偏强','reason':f"中美竞争 {cat_counts['中美竞争']} 条；军事 {cat_counts['军事']} 条；能源 {cat_counts['能源安全']} 条。",'time':stamp,'evidence':'今日分类统计'})
alerts=alerts[:12]
ALERTS.write_text(json.dumps({'updated':stamp,'alerts':alerts},ensure_ascii=False,indent=2),encoding='utf-8')
